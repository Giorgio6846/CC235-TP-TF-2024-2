...
def saveFaces(facePath, depthPath, face1Path, depth1Path):
    ...
    if images != None:
        ...
        cv2.imwrite(f"./data/tmp/imgProc/F1", faceRGB)
        cv2.imwrite(f"./data/tmp/imgProc/D1", images[1])
    ...
    if images != None:
        ...
        cv2.imwrite(f"./data/tmp/imgProc/F2", faceRGB)
        cv2.imwrite(f"./data/tmp/imgProc/D2", images[1])
